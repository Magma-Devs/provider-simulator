...
class TestScenario:
    ...
    def test_post_response_applied_shows_default_family_when_field_omitted(self, sim):
        ...
        status, body = _post(_ctrl(sim, "/scenario"), {"providers": {"3": {"mode": "success"}}})
        assert status == 200
        assert "applied" in body
    ...
    def test_scenario_success_response_shape_includes_applied(self, sim):
        ...
        status, body = _post(_ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "error"}}})
        assert status == 200
...
class TestErrorStubs:
    ...
    def test_per_method_http_status_override(self, sim):
        ...
        _post(
            ...
            {
                "providers": {
                    "1": {"responses": {"eth_call": {"error_stub": "revert", "http_status": 200}}}
                }
            },
        )
        status, body = _rpc(sim["provider1"], "eth_call")
...
class TestReset:
    ...
    def test_reset_restores_scenario_to_defaults(self, sim):
        _post(
            _ctrl(sim, "/scenario"),
            {"providers": {"1": {"mode": "error", "latency_ms": 500, "error_probability": 0.9}}},
        )
...
class TestScenarioValidation:
    ...
    def test_valid_scenario_still_returns_200(self, sim):
        ...
        status, body = _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "error", "latency_ms": 10}}}
        )
        assert status == 200
        assert body["status"] == "ok"
    ...
    def test_unknown_field_returns_400(self, sim):
        ...
        status, body = _post(_ctrl(sim, "/scenario"), {"providers": {"1": {"latencyms": 500}}})
        assert status == 400
        assert "latencyms" in body["error"]

    def test_invalid_mode_returns_400(self, sim):
        status, body = _post(_ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "boom"}}})
        assert status == 400
    ...
    def test_invalid_corruption_mode_returns_400(self, sim):
        status, body = _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"corruption_mode": "shred"}}}
        )
        assert status == 400
    ...
    def test_error_probability_out_of_range_returns_400(self, sim):
        status, body = _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"error_probability": 2.0}}}
        )
        assert status == 400
    ...
    def test_negative_latency_returns_400(self, sim):
        ...
        status, body = _post(_ctrl(sim, "/scenario"), {"providers": {"1": {"latency_ms": -5}}})
        assert status == 400
    ...
    def test_invalid_then_mode_returns_400(self, sim):
        status, body = _post(_ctrl(sim, "/scenario"), {"providers": {"1": {"then_mode": "sucess"}}})
        assert status == 400
        assert "then_mode" in body["error"]

    def test_negative_fail_first_n_returns_400(self, sim):
        status, body = _post(_ctrl(sim, "/scenario"), {"providers": {"1": {"fail_first_n": -1}}})
        assert status == 400
        assert "fail_first_n" in body["error"]

    def test_invalid_solana_unknown_method_mode_returns_400(self, sim):
        status, body = _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"solana_unknown_method_mode": "bogus"}}}
        )
        assert status == 400
    ...
    def test_invalid_scenario_does_not_mutate_other_providers(self, sim):
        ...
        status, _ = _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "error"}, "2": {"mode": "boom"}}}
        )
        assert status == 400
...
class TestHttpStatusInSuccessMode:
    ...
    def test_success_mode_with_custom_http_status(self, sim):
        _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "success", "http_status": 502}}}
        )
        status, body = _rpc(sim["provider1"], "eth_blockNumber")
...
class TestBlocksBehind:
    ...
    def test_blocks_behind_shifts_eth_blockNumber(self, sim):
        _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "success", "blocks_behind": 100}}}
        )
        _, body = _rpc(sim["provider1"], "eth_blockNumber")
    ...
    def test_blocks_ahead_via_negative_value(self, sim):
        _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "success", "blocks_behind": -50}}}
        )
        _, body = _rpc(sim["provider1"], "eth_blockNumber")
    ...
    def test_eth_get_block_by_number_latest_respects_blocks_behind(self, sim):
        _post(
            _ctrl(sim, "/scenario"), {"providers": {"1": {"mode": "success", "blocks_behind": 25}}}
        )
        _, body = _rpc(sim["provider1"], "eth_getBlockByNumber", ["latest", False])
...
class TestDropConnection:
    ...
    def test_drop_before_headers_no_response(self, sim):
        _post(
            _ctrl(sim, "/scenario"),
            {"providers": {"1": {"mode": "drop_connection", "drop_at": "before_headers"}}},
        )
    ...
    def test_drop_after_headers_incomplete_read(self, sim):
        _post(
            _ctrl(sim, "/scenario"),
            {"providers": {"1": {"mode": "drop_connection", "drop_at": "after_headers"}}},
        )
        result = self._send_and_capture_error(sim["provider1"])
        assert result != "OK", "expected error from connection drop, got valid response"
    ...
    def test_drop_mid_body_truncated(self, sim):
        _post(
            _ctrl(sim, "/scenario"),
            {"providers": {"1": {"mode": "drop_connection", "drop_at": "mid_body"}}},
        )
        result = self._send_and_capture_error(sim["provider1"])
        assert result != "OK", "expected error from connection drop, got valid response"

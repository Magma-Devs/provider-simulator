...
- restarts `deployment/provider-simulator` in namespace `smart-router` — but only when a fresh image was built this run, the manifests actually changed (`kubectl diff`), or `FORCE_RESTART=true` is set. A no-op re-run (`SKIP_BUILD=true`, nothing changed) skips the restart and just confirms the deployment is healthy.
...
```zsh
helm upgrade smart-router ... -n smart-router
```
